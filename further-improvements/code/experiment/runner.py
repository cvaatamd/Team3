...
def load_plan(path: Path) -> ExperimentPlan:
    ...
    return ExperimentPlan(
        ...
        shuffle_external_seed=raw.get("shuffle_external_seed"),  # Patch 3 negative control
    )
...
def plan_from_manifest(path: Path) -> ExperimentPlan:
    ...
    *this* results dir, with no dependency on which plan YAML happened to be the default. Useful
    ...
    return ExperimentPlan(
        ...
        shuffle_external_seed=jobs[0].shuffle_external_seed,  # Patch 3 negative control
    )
...
def run_one(
    ...
    *,
    ...
    # Patch 1 (further-improvements): persist native-scale predictions alongside the results so
    # the official RAE and molecule-level bootstrap can be recomputed offline (no retraining).
    preds_dir = str((results_dir / "preds").resolve())
    spec = TrainSpec(
        pool=pool, test_df=data.test, arm=job.arm, n=job.n, seed=job.seed,
        mechanism=job.mechanism, rae_definition=job.rae_definition,
        preds_dir=preds_dir, job_id=job.job_id,
    )
    result = ma.run(spec)
