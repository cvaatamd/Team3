...
def rae(
    ...
    *,
    ...
    if definition == "official":
        # Patch 2 (further-improvements): the OpenADMET-ExpansionRx headline metric is a
        # range-normalized absolute error — per-endpoint MAE divided by that endpoint's dynamic
        # range on the test set, then macro-averaged (see `ma_rae`). This matches our
        # `range_normalized` definition; we keep it as a distinct, explicitly-named branch so the
        # report can show both side by side and a reviewer can confirm they agree.
        #
        # VERIFY before trusting (Plan.md §1/§9): diff this against the scoring code in the
        # challenge HF Space (`openadmet/OpenADMET-ExpansionRx-Challenge`). If the official code
        # (a) operates in transformed/log space, (b) uses a robust/percentile range, or
        # (c) uses fixed published per-endpoint ranges, replace the `rng` line below accordingly.
        rng = test_range if test_range is not None else float(y_true.max() - y_true.min())
        return float(mae / rng) if rng > 0 else float("nan")
