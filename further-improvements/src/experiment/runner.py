...
def load_plan(path: Path) -> ExperimentPlan:
    ...
    return ExperimentPlan(
        ...
        shuffle_external_seed=raw.get("shuffle_external_seed", None),  # Patch 3
    )
...
def plan_from_manifest(path: Path) -> ExperimentPlan:
    ...
    *this* results dir, with no dependency on which plan YAML happened to be the default. Useful
    ...
    return ExperimentPlan(
        ...
        shuffle_external_seed=jobs[0].shuffle_external_seed,
    )
...
def run_one(
    ...
    *,
    ...
    spec = TrainSpec(
        ...
        # Patch 1: persist native-scale predictions next to the run JSONs.
