...
class PoolRequest(BaseModel):
    ...
    # Patch 3 (further-improvements): negative control. When set, the external source's labels
    # are permuted under this seed before harmonization — keeps the marginal distribution but
    # destroys the structure-activity link, so any "lift" here is pure regularization.
...
class JobSpec(BaseModel):
    ...
    shuffle_external_seed: Optional[int] = None  # Patch 3: negative control (None == off)
