...
def in_process_dispatcher(
    ...
    *,
    ...
    `preds_dir` (Patch 1): when set, each job persists native-scale (y_true, y_pred).
    """
    ...
    def _dispatch(jobs: list[JobSpec]) -> list[EvalResult]:
        ...
        for j in jobs:
            ...
            spec = TrainSpec(
                ...
                preds_dir=preds_dir, job_id=j.job_id,
            )
...
def _pool_request_from_job(j: JobSpec) -> PoolRequest:
    return PoolRequest(
        ...
        shuffle_external_seed=j.shuffle_external_seed,
    )
