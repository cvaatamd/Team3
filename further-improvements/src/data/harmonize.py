...
def harmonize_external(
    ...
    *,
    ...
    Patch 3 (further-improvements): if `shuffle_seed` is not None, the harmonized source labels
    are permuted across molecules under that seed (negative control). This preserves the source's
    marginal distribution but breaks the structure↔activity mapping, so a model can only exploit
    the extra column as generic regularization — any lift relative to the *real* external arm is
    therefore not transferable signal.
    """
    src = source.loader()
    ...
    shuffled = False
    if shuffle_seed is not None and len(src) > 1:
        rng = np.random.default_rng(shuffle_seed)
        src[aux_col] = src[aux_col].to_numpy()[rng.permutation(len(src))]
        shuffled = True
    ...
    if shuffled:
        notes.append(f"NEGATIVE_CONTROL=shuffled(seed={shuffle_seed})")
    if mapping.notes:
